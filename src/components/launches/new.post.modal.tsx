...
export function NewPostModal({ onClose, onPostCreated, initialDate, initialPost }: {
  ...
  const {
    postToLinkedIn, isPosting: postingToLI, result: liPostResult, message: liPostMsg, isConnected: liConnected,
  } = useLinkedInPost();
      ...
      <div className={`${styles.modalWrap} ${activeAgent ? styles.modalWrapExpanded : ''}`}>
        ...
        <div className={styles.modal}>
          ...
          <div className={styles.footer}>
            {/* LinkedIn post feedback */}
            {liPostMsg && (
              <div className={liPostResult === 'success' ? styles.successMsg : styles.errorMsg} style={{ flex: 1, marginRight: 8 }}>
            ...
            {showLinkedInBtn && (
              <button type="button" className={styles.submitBtn}
                ...
                {postingToLI ? 'Posting...' : liPostResult === 'success' ? '✓ Posted!' : '🚀 Post to LinkedIn'}
              </button>
            )}
            <button type="button" className={styles.submitBtn}
